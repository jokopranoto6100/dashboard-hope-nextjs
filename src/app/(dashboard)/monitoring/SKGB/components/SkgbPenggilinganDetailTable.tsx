...
import { SkgbPenggilinganDetailData } from '@/hooks/useSkgbPenggilinganData';
...
interface SkgbPenggilinganDetailTableProps {
  kabupatenName: string;
  data: SkgbPenggilinganDetailData[];
  totals: {
    target_utama: number;
    cadangan: number;
    realisasi: number;
...
const TableSkeleton = ({ columns }: { columns: ColumnDef<SkgbPenggilinganDetailData, unknown>[] }) => (
...
export function SkgbPenggilinganDetailTable({ kabupatenName, data, totals, onBack, isLoading, lastUpdated, jadwal }: SkgbPenggilinganDetailTableProps) {
  ...
  const columns: ColumnDef<SkgbPenggilinganDetailData>[] = useMemo(() => [
    {
    ...
    },
    {
      ...
      header: ({ column }) => (
        <Button variant="ghost" onClick={() => column.toggleSorting(column.getIsSorted() === "asc")} className="h-8 p-1 font-semibold hover:bg-transparent">
          Desa/Kelurahan <ArrowUpDown className="ml-2 h-3 w-3" />
        </Button>
      ),
      cell: ({ row }) => (
        <div className="font-medium text-left pl-2">
          {capitalizeWords(row.getValue('desa'))}
        </div>
      ),
      size: 150,
      enableSorting: true,
          ...
          <div className="flex flex-col gap-4">
            ...
            <div>
              <CardTitle className="flex items-center gap-2">
                Detail SKGB Penggilingan - {kabupatenName}
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex-grow">
              <CardTitle>Detail SKGB Penggilingan - {kabupatenName}</CardTitle>
              <CardDescription>
                Data per kecamatan dan desa
                {lastUpdated && (
