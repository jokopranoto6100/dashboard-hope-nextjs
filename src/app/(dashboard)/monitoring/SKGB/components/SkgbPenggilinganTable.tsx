...
interface SkgbPenggilinganTableProps {
  title: string;
  description: string;
  data: SkgbPenggilinganDistrictData[];
  totals: {
    target_utama: number;
    cadangan: number;
    realisasi: number;
  ...
  onRowClick?: (districtData: SkgbPenggilinganDistrictData) => void;
  isLoading: boolean;
...
export function SkgbPenggilinganTable({ title, description, data, totals, onRowClick, isLoading, lastUpdated, jadwal }: SkgbPenggilinganTableProps) {
  ...
  const countdownStatus = useMemo(() => {
    ...
    // For SKGB Penggilingan subkegiatan, get jadwal items from the subkegiatan itself
    // The jadwal passed here should be the Penggilingan subkegiatan, not the main SKGB kegiatan
  ...
  const columns: ColumnDef<SkgbPenggilinganDistrictData>[] = useMemo(() => [
    {
