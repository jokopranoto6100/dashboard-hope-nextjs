...
export default function UbinanMonitoringPage() {
  ...
  const padiColumns: ColumnDef<PadiDataRow>[] = React.useMemo(() => {
    // ✅ PERUBAIKAN: Definisikan kolom persentase secara terpisah untuk dipindahkan
    ...
    const baseColumns: ColumnDef<PadiDataRow>[] = [
      ...
      // Kolom persentase dipindahkan ke sini
    ...
    // ✅ PERUBAIKAN: Hapus kolom persentase dari sini karena sudah dipindah
    ...
    // ✅ PERUBAIKAN: Susun ulang urutan kolom
    return [
      ...baseColumns.slice(0, 4), // nmkab, targetUtama, cadangan, realisasi
      persentaseColumn,
      baseColumns[4], // lewatPanen
  ...
  const currentPadiSkeletonColumns = React.useMemo(() => {
    ...
    if (!activePadiColumns.length && processedPadiData?.length) {
        return padiColumns;
    }
  ...
  const palawijaColumns: ColumnDef<PalawijaDataRow>[] = React.useMemo(() => {
    ...
    return [
      ...baseColumns, 
      ...(isRealisasiExpanded ? realisasiDetailColumns : realisasiSummaryColumn),
      persentaseColumn // Pindahkan ke sini
    ];
  }, [isRealisasiExpanded]);
  ...
  return (
    // ✅ PERUBAIKAN: Ganti container menjadi div biasa, hapus padding bawaan page.tsx
    // Class 'container' akan ditambahkan di layout utama jika diperlukan.
    <div className="flex flex-col gap-8">
      ...
      <Card>
        ...
        <CardContent>
          {loadingPadi && <PadiTableSkeleton columns={currentPadiSkeletonColumns.length > 0 ? currentPadiSkeletonColumns : padiColumns} />}
                      ...
                      {padiTable.getVisibleLeafColumns().map(col => {
                        ...
                        return (
                          <TableCell
                            ...
                            className={
                              columnId === 'nmkab' ? 'text-left font-bold' : 'text-center font-bold'
                            }
                            style={{ width: col.getSize(), minWidth: (col.columnDef as ColumnDef<PadiDataRow, unknown>).minSize ? `${(col.columnDef as ColumnDef<PadiDataRow, unknown>).minSize}px` : undefined }}
                          >
                            {isPercentage ? (() => {
      ...
      <Card>
        ...
        <CardContent>
          {loadingPalawija && <PalawijaTableSkeletonComponent columns={currentPalawijaSkeletonColumns.length > 0 ? currentPalawijaSkeletonColumns : palawijaColumns} />}
                      ...
                      {palawijaTable.getVisibleLeafColumns().map(col => {
                        ...
                        return (
                          <TableCell
                            ...
                            style={{ width: col.getSize(), minWidth: (col.columnDef as ColumnDef<PalawijaDataRow, unknown>).minSize ? `${(col.columnDef as ColumnDef<PalawijaDataRow, unknown>).minSize}px` : undefined }}
                          >
                            {isPercentage ? (() => {
