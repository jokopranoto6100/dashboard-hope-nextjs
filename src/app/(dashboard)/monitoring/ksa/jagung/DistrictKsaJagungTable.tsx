...
export function DistrictKsaJagungTable({ title, description, data, totals, uniqueStatusNames, onRowClick, isLoading, jadwal, displayMonth }: DistrictKsaJagungTableProps) {
  ...
  const countdownStatus = React.useMemo(() => {
    ...
    console.log("🔍 KSA Jagung District Countdown Debug:", {
      displayMonth,
      jadwalData: jadwal,
    ...
    console.log("📅 Jagung District Today:", today.toISOString().split('T')[0]);
    ...
    const activeOrUpcomingSchedules = allJadwalItems.filter(item => {
      ...
      console.log("📊 Jagung District Schedule analysis:", {
        nama: item.nama,
    ...
    console.log("✅ Jagung District Active/Upcoming schedules:", activeOrUpcomingSchedules);

    if (activeOrUpcomingSchedules.length === 0) {
      console.log("❌ Jagung District No active or upcoming schedules found");
      return null;
    }
    ...
    if (displayMonth && displayMonth !== "Semua") {
      ...
      if (monthSpecificSchedules.length > 0) {
        ...
        console.log("✅ Jagung District Found month-specific schedules:", monthSpecificSchedules);
      } else {
        console.log("⚠️ Jagung District No schedules for selected month, using all active/upcoming");
      }
    }
    ...
    if (currentlyActive.length > 0) {
      ...
      console.log("📊 Jagung District Active schedule - Days calculation:", {
        daysLeft,
    ...
    if (upcoming.length > 0) {
      ...
      console.log("📊 Jagung District Upcoming schedule - Days calculation:", {
        daysUntil,
        todayStr: today.toISOString().split('T')[0],
    ...
    console.log("❌ Jagung District No relevant schedules found");
    return null;
  }, [jadwal, displayMonth]);
