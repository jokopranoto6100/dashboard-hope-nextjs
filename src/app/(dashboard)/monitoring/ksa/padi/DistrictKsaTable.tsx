...
export function DistrictKsaTable({ title, description, data, totals, uniqueStatusNames, onRowClick, isLoading, jadwal, displayMonth }: DistrictKsaTableProps) {
  ...
  const countdownStatus = React.useMemo(() => {
    ...
    console.log("🔍 KSA District Countdown Debug:", {
      displayMonth,
      jadwalData: jadwal,
    ...
    const activeOrUpcomingSchedules = allJadwalItems.filter(item => {
      ...
      console.log("� District Schedule analysis:", {
        nama: item.nama,
    ...
    console.log("✅ District Active/Upcoming schedules:", activeOrUpcomingSchedules);

    if (activeOrUpcomingSchedules.length === 0) {
      console.log("❌ No active or upcoming schedules found");
      return null;
    }
    ...
    if (displayMonth && displayMonth !== "Semua") {
      ...
      if (monthSpecificSchedules.length > 0) {
        ...
        console.log("✅ District Found month-specific schedules:", monthSpecificSchedules);
      } else {
        console.log("⚠️ District No schedules for selected month, using all active/upcoming");
      }
    }
    ...
    if (currentlyActive.length > 0) {
      ...
      console.log("📊 District Active schedule - Days calculation:", {
        daysLeft,
    ...
    if (upcoming.length > 0) {
      ...
      console.log("📊 District Upcoming schedule - Days calculation:", {
        daysUntil,
        todayStr: today.toISOString().split('T')[0],
    ...
    console.log("❌ District No relevant schedules found");
    return null;
  }, [jadwal, displayMonth]);
