...
export function NamaKsaTable({ title, description, data, totals, uniqueStatusNames, kabupatenName, isLoading, onGenerateBaClick, jadwal, displayMonth }: NamaKsaTableProps) {
  ...
  const countdownStatus = React.useMemo(() => {
    ...
    console.log("🔍 KSA Nama Countdown Debug:", {
      displayMonth,
      jadwalData: jadwal,
    ...
    console.log("✅ Active/Upcoming schedules:", activeOrUpcomingSchedules);
    ...
    if (displayMonth && displayMonth !== "Semua") {
      ...
      if (monthSpecificSchedules.length > 0) {
        ...
        console.log("✅ Found month-specific schedules:", monthSpecificSchedules);
      } else {
        console.log("⚠️ No schedules for selected month, using all active/upcoming");
      }
    }
    ...
    if (currentlyActive.length > 0) {
      ...
      console.log("📊 Active schedule - Days calculation:", {
        daysLeft,
    ...
    if (upcoming.length > 0) {
      ...
      console.log("📊 Upcoming schedule - Days calculation:", {
        daysUntil,
        todayStr: today.toISOString().split('T')[0],
    ...
    console.log("❌ No relevant schedules found");
    return null;
  }, [jadwal, displayMonth]);
