...
import * as xlsx from 'xlsx'; // BARU: Import xlsx untuk parsing di client

// Server Action ini akan kita panggil
import { uploadKsaAction } from "./_actions"; 

// BARU: Mapping Kabupaten untuk digunakan di client
...
export function KsaUploader() {
  ...
  const [isAnalyzing, setIsAnalyzing] = useState(false); // BARU: State untuk proses analisis file
  ...
  // --- FUNGSI INI DIPATCH SECARA KESELURUHAN ---
        ...
        for (const record of jsonData) {
          if (record.tanggal && record['id segmen']) {
            ...
            if (!isNaN(date.getTime())) {
              ...
              monthSet.add(date.getMonth() + 1); // getMonth() 0-11
  ...
  const isLoading = isPending || isAnalyzing; // BARU: Kondisi loading gabungan
  ...
  return (
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      <div className="w-full max-w-2xl mx-auto p-2 space-y-6">
        ...
        {/* Tombol Aksi - Logika onClick diubah */}
      ...
      {/* Konten Modal Konfirmasi - Tampilan Disesuaikan */}
